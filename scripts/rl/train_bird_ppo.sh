...
export BIRD_DB_ROOT_DEV="${BIRD_DB_ROOT_DEV:-${LSE_BIRD_DATA_ROOT}/dev_data/dev_databases}"
export BIRD_DB_ROOT_TRAIN="${BIRD_DB_ROOT_TRAIN:-${LSE_BIRD_DATA_ROOT}/train_data/train_databases}"

export LSE_RL_TRAIN_FILES="${LSE_RL_TRAIN_FILES:-${REPO_ROOT}/logs/bird_data/bird}"
export LSE_RL_VAL_FILES="${LSE_RL_VAL_FILES:-${REPO_ROOT}/logs/bird_data_test/bird}"

export NUM_API_WORKERS="${NUM_API_WORKERS:-32}"
export NUM_SQL_WORKERS="${NUM_SQL_WORKERS:-64}"
export SEM_MAX="${SEM_MAX:-32}"
export GEN_MAX="${GEN_MAX:-64}"
export SQL_TIMEOUT="${SQL_TIMEOUT:-30}"
export SQL_CACHE="${SQL_CACHE:-1}"
...
REWARD_MODEL="${REWARD_MODEL:-Qwen/Qwen3-4B-Instruct-2507}"
...
python -m verl.trainer.main_lse_ppo \
  ...
  data.max_prompt_length="${MAX_PROMPT_LENGTH:-65536}" \
  ...
  trainer.project_name="${RL_PROJECT_NAME:-bird_lse}" \
