...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if "token_level_scores" in data.batch:  # optional for custom estimators (e.g., logit_delta)
            adv_kwargs["token_level_scores"] = data.batch["token_level_scores"]
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # reward loop can be used without spawning RewardModel workers
        self.use_rm = self.config.reward_model.enable or need_reward_model(self.role_worker_mapping)
    ...
    def _validate(self, merged: bool = False):
        ...
        # Group validation scores by db/subject for curriculum-style reporting.
        # Keyed by (data_source, db_id) where db_id is Bird DB or MMLU subject.
        db_score_buckets: dict[tuple[str, str], list[float]] = defaultdict(list)
        ...
        for test_data in self.val_dataloader:
            ...
            # In shared-GPU-pool mode (reward_model.enable_resource_pool=False), the AgentLoop cannot
            # compute rewards during generation because it has no reward_router_address. In that case,
            # we must compute rm_scores here via RewardLoopManager; otherwise validation falls back to
            # the legacy reward_fn path (self.val_reward_fn), which may not have access to the router
            # address required by custom reward functions (e.g., lse_reward_fn using /v1/chat/completions).
            if self.use_reward_loop and self.use_rm and "rm_scores" not in test_batch.batch.keys():
                reward_loop_mgr = getattr(self, "reward_loop_manager", None)
                if reward_loop_mgr is not None:
                    rm_scores_dp = reward_loop_mgr.compute_rm_score(test_batch)
                    test_batch = test_batch.union(rm_scores_dp)
                    del rm_scores_dp
            ...
            # Per-db/subject score breakdown (validation only).
            # Prefer raw `score` from reward_extra_info if present; otherwise fall back to summed reward_tensor.
            score_for_grouping = None
            if isinstance(reward_extra_info, dict) and "score" in reward_extra_info:
                score_vals = reward_extra_info.get("score")
                if isinstance(score_vals, np.ndarray):
                    score_for_grouping = score_vals.tolist()
                elif isinstance(score_vals, list):
                    score_for_grouping = score_vals

            if score_for_grouping is None or len(score_for_grouping) != len(scores):
                score_for_grouping = scores

            extra_infos = test_batch.non_tensor_batch.get("extra_info", None)
            data_sources = test_batch.non_tensor_batch.get("data_source", None)
            if extra_infos is not None and data_sources is not None:
                for ds, info, val in zip(data_sources, extra_infos, score_for_grouping, strict=False):
                    if not isinstance(info, dict):
                        continue
                    db_id = info.get("db_id", None)
                    if db_id is None:
                        continue
                    try:
                        val_f = float(val)
                    ...
                    if not np.isfinite(val_f):
                        continue
                    db_score_buckets[(str(ds), str(db_id))].append(val_f)

            reward_extra_infos_dict["reward"].extend(scores)
            for key, values in reward_extra_info.items():
                # TODO: review
                # Avoid double-counting: `reward` is reserved for the scalar metric used by
                # validation aggregation (we already appended `scores` above).
                # Custom reward functions (e.g., lse_reward_fn) may also return a field named
                # `reward` (often a delta vs baseline). Preserve it under a different name.
                out_key = "reward_delta" if key == "reward" else key
                if out_key not in reward_extra_infos_dict:
                    reward_extra_infos_dict[out_key] = []
                if isinstance(values, np.ndarray):
                    reward_extra_infos_dict[out_key].extend(values.tolist())
                else:
                    reward_extra_infos_dict[out_key].extend(values if isinstance(values, list) else [values])
        ...
        # Add per-db/subject min/max/mean + avg(max) across dbs.
        if db_score_buckets:
            per_ds_maxima: dict[str, list[float]] = defaultdict(list)
            for (ds, db_id), vals in db_score_buckets.items():
                if not vals:
                    continue
                arr = np.asarray(vals, dtype=np.float32)
                db_key = db_id.replace("/", "_").replace(" ", "_")
                metric_dict[f"val-db/{ds}/{db_key}/score/mean"] = float(arr.mean())
                metric_dict[f"val-db/{ds}/{db_key}/score/max"] = float(arr.max())
                metric_dict[f"val-db/{ds}/{db_key}/score/min"] = float(arr.min())
                per_ds_maxima[ds].append(float(arr.max()))

            # avg of per-db maxima (per data_source + global)
            all_maxima: list[float] = []
            for ds, maxima in per_ds_maxima.items():
                if maxima:
                    metric_dict[f"val-db/{ds}/average_max"] = float(np.mean(maxima))
                    all_maxima.extend(maxima)
            if all_maxima:
                metric_dict["val-db/average_max"] = float(np.mean(all_maxima))
    ...
    def init_workers(self):
        ...
        if not self.use_reward_loop:
            ...
            if not can_reward_loop_parallelize:
                ...
                # NOTE: In "shared GPU pool" mode, reward model and actor share the same GPUs.
                # We create a RewardLoopManager here (synchronous reward loop) so RM requests go through
                # the reward router and we can wake/sleep the reward model around scoring.
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # Build token-level rewards from reward_extra_info if provided.
                        reward_values = None
                        if reward_extra_infos_dict and "reward" in reward_extra_infos_dict:
                            reward_values = reward_extra_infos_dict["reward"]
                        elif "reward" in batch.non_tensor_batch:
                            reward_values = batch.non_tensor_batch["reward"]

                        if reward_values is not None:
                            reward_values_tensor = torch.as_tensor(
                                reward_values, dtype=torch.float32, device=batch.batch["response_mask"].device
                            ).view(-1)
                            token_level_rewards = torch.zeros_like(
                                batch.batch["response_mask"], dtype=torch.float32
                            )
                            response_lengths = batch.batch["response_mask"].sum(dim=-1) - 1
                            response_lengths = torch.clamp(response_lengths, min=0)
                            idx = torch.arange(token_level_rewards.size(0), device=token_level_rewards.device)
                            valid = response_lengths < token_level_rewards.size(1)
                            token_level_rewards[idx[valid], response_lengths[valid]] = reward_values_tensor[valid]
                        ...
                        if self.config.algorithm.use_kl_in_reward:
                            if "token_level_rewards" in batch.batch:
                                ...
                                kld = kld * response_mask
                                beta = self.kl_ctrl_in_reward.value
                                batch.batch["token_level_rewards"] = batch.batch["token_level_rewards"] - beta * kld
                                current_kl = masked_mean(kld, mask=response_mask, axis=-1)
                                current_kl = torch.mean(current_kl, dim=0).item()
                                batch_size = batch.batch.batch_size[0]
                                self.kl_ctrl_in_reward.update(current_kl=current_kl, n_steps=batch_size)
                                metrics.update(
                                    {
