...
def compute_delta_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    config: Optional[AlgoConfig] = None,
    **kwargs,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Delta advantage: broadcast outcome reward across response tokens."""
    del kwargs
    with torch.no_grad():
        scores = token_level_rewards.sum(dim=-1)

        std_norm = bool(config.get("std_norm", False)) if config is not None else False
        whitening = bool(config.get("whitening", False)) if config is not None else False
        if std_norm and whitening:
            raise ValueError("Only one of `algorithm.std_norm` and `algorithm.whitening` can be True.")

        if std_norm or whitening:
            valid_seq_mask = response_mask.sum(dim=-1) > 0
            if torch.any(valid_seq_mask):
                valid_scores = scores[valid_seq_mask]
                # std/mean are undefined for a single element with unbiased=True (PyTorch default)
                if valid_scores.numel() > 1:
                    std = valid_scores.std()
                    if whitening:
                        mean = valid_scores.mean()
                        scores = scores.clone()
                        scores[valid_seq_mask] = (scores[valid_seq_mask] - mean) / (std + 1e-8)
                    else:
                        scores = scores.clone()
                        scores[valid_seq_mask] = scores[valid_seq_mask] / (std + 1e-8)
        advantages = scores.unsqueeze(-1) * response_mask
    return advantages, advantages


@register_adv_est("logit_delta")
def compute_logit_delta_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    **kwargs,
    ...
    """Logit-delta advantage from RL2 (requires both rewards and scores)."""
    del kwargs
    if token_level_scores is None:
        raise ValueError("logit_delta requires token_level_scores")
    with torch.no_grad():
        rewards = token_level_rewards.sum(dim=-1)
        scores = token_level_scores.sum(dim=-1)
        old_performance = scores - rewards
        top = scores * (1 - old_performance)
        bottom = old_performance * (1 - scores) + 1e-6
        advantages_scalar = torch.log(top / bottom)
        advantages_scalar = torch.where(scores == -1, torch.full_like(advantages_scalar, -1.0), advantages_scalar)
        ...
        if std_norm or whitening:
            valid_seq_mask = (response_mask.sum(dim=-1) > 0) & (scores != -1) & torch.isfinite(advantages_scalar)
            if torch.any(valid_seq_mask):
                valid_adv = advantages_scalar[valid_seq_mask]
                # std/mean are undefined for a single element with unbiased=True (PyTorch default)
                if valid_adv.numel() > 1:
                    std = valid_adv.std()
                    advantages_scalar = advantages_scalar.clone()
                    if whitening:
                        mean = valid_adv.mean()
                        advantages_scalar[valid_seq_mask] = (advantages_scalar[valid_seq_mask] - mean) / (std + 1e-8)
                    else:
                        advantages_scalar[valid_seq_mask] = advantages_scalar[valid_seq_mask] / (std + 1e-8)
