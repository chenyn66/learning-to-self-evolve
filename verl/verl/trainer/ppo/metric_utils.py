...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - critic/advantages/mean, max, min: Statistics about per-response advantages
            - critic/advantages_token/mean, max, min: Statistics about token-level advantages
    ...
    reward_pos_rate = torch.mean((non_aborted_sequence_reward > 0).float()).detach().item()
    reward_nonneg_rate = torch.mean((non_aborted_sequence_reward >= 0).float()).detach().item()

    # NOTE:
    # - Token-level advantages are typically defined per response token.
    # - For logging "per response" stats, we first reduce advantages within each response (masked mean)
    #   and then aggregate across responses. This avoids response-length weighting in the batch mean.
    advantages_per_response = verl_F.masked_mean(advantages, response_mask, axis=-1)
    non_aborted_advantages_per_response = advantages_per_response[non_aborted_mask]

    adv_mean = torch.mean(non_aborted_advantages_per_response).detach().item()
    adv_max = torch.max(non_aborted_advantages_per_response).detach().item()
    adv_min = torch.min(non_aborted_advantages_per_response).detach().item()

    # Keep token-level advantage stats for debugging/compatibility (these are response-length weighted).
    ...
    metrics = {
        ...
        "critic/rewards/pos_rate": reward_pos_rate,
        "critic/rewards/nonneg_rate": reward_nonneg_rate,
        # adv (per response)
        "critic/advantages/mean": adv_mean,
        "critic/advantages/max": adv_max,
        "critic/advantages/min": adv_min,
        # adv (token-level)
        "critic/advantages_token/mean": torch.mean(valid_adv).detach().item(),
        "critic/advantages_token/max": torch.max(valid_adv).detach().item(),
        "critic/advantages_token/min": torch.min(valid_adv).detach().item(),
        # returns
    ...
    # Dataset-specific optional metrics (safe no-op if keys are missing).
    # LSE curriculum uses `extra_info.improve_potential`; log its batch mean if present.
    if "extra_info" in batch.non_tensor_batch:
        extra_infos = batch.non_tensor_batch["extra_info"]
        improve_potentials: list[float] = []
        for info in extra_infos:
            if not isinstance(info, dict):
                continue
            val = info.get("improve_potential")
            if val is None:
                continue
            try:
                improve_potentials.append(float(val))
            except (TypeError, ValueError):
                continue
        if improve_potentials:
            metrics["data/improve_potential/mean"] = float(np.mean(improve_potentials))
            metrics["data/improve_potential/max"] = float(np.max(improve_potentials))
            metrics["data/improve_potential/min"] = float(np.min(improve_potentials))
