...
class RewardModelManager:
    ...
    def _initialize_llm_servers(self):
        # NOTE: Some configs don't include `pipeline_model_parallel_size` (older structs).
        # Use `.get(..., 1)` to avoid ConfigAttributeError while still supporting pipeline MP.
        pipeline_mp_size = self.config.rollout.get("pipeline_model_parallel_size", 1)
        rollout_world_size = (
            self.config.rollout.tensor_model_parallel_size * self.config.rollout.data_parallel_size * pipeline_mp_size
        )
        world_size = (
