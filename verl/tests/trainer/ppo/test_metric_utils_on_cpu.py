...
class TestComputeDataMetrics(unittest.TestCase):
    ...
    def test_advantage_metrics_logged_per_response(self):
        """
        Advantages should be logged per response (per-sequence) rather than token-weighted.

        This matters when response lengths vary: token-weighted averaging biases metrics toward
        longer responses, even if the per-response advantage distribution is different.
        """
        batch = MagicMock()
        batch.non_tensor_batch = {}
        batch.batch = {
            # 2 samples, response_length=2
            "responses": torch.zeros((2, 2)),
            # attention_mask: 2 prompt tokens + 2 response tokens
            # sample 2 has only 1 valid response token
            "attention_mask": torch.tensor([[1, 1, 1, 1], [1, 1, 1, 0]]),
            "response_mask": torch.tensor([[1, 1], [1, 0]]),
            # ensure masked tokens are zeros to mimic real batches
            "token_level_scores": torch.tensor([[1.0, 2.0], [3.0, 0.0]]),
            "token_level_rewards": torch.tensor([[0.5, 1.0], [1.5, 0.0]]),
            "advantages": torch.tensor([[0.1, 0.2], [0.3, 0.0]]),
            "returns": torch.tensor([[1.1, 1.2], [1.3, 0.0]]),
        }

        metrics = compute_data_metrics(batch, use_critic=False)

        # Per-response advantage is the masked mean within each response:
        # sample1: (0.1 + 0.2)/2 = 0.15
        # sample2: (0.3)/1 = 0.3
        # mean over responses: (0.15 + 0.3)/2 = 0.225
        self.assertAlmostEqual(metrics["critic/advantages/mean"], 0.225, places=6)

        # Token-level advantage mean would be over valid tokens: (0.1 + 0.2 + 0.3)/3 = 0.2
        self.assertAlmostEqual(metrics["critic/advantages_token/mean"], 0.2, places=6)
